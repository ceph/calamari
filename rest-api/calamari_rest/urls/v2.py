...
urlpatterns = patterns(
    ...
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/crush_rule$', calamari_rest.views.v2.CrushRuleViewSet.as_view({'get': 'list'}),
        name='cluster-crush_rule-list'),
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/pool$', calamari_rest.views.v2.PoolViewSet.as_view({'get': 'list', 'post': 'create'}),
        name='cluster-pool-list'),
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/pool/(?P<pool_id>\d+)$', calamari_rest.views.v2.PoolViewSet.as_view({
        'get': 'retrieve',
        'patch': 'update',
        'delete': 'destroy'}),
    ...
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/osd/command$', calamari_rest.views.v2.OsdViewSet.as_view({'get': 'valid_commands'})),
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/osd/command/implemented$', calamari_rest.views.v2.OsdViewSet.as_view(
...
# Direct access to SyncObjects, DerivedObjects, graphite stats
    ...
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/server/(?P<fqdn>[a-zA-Z0-9-\.]+)$',
        calamari_rest.views.v2.ServerClusterViewSet.as_view({'get': 'retrieve'}), name='cluster-server-detail'),

    # Ceph configuration settings
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/config$', calamari_rest.views.v2.ConfigViewSet.as_view({'get': 'list'})),
